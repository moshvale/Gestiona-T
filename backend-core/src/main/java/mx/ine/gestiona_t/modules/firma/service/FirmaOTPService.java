...
public class FirmaOTPService {
    ...
    private static final Logger log = LoggerFactory.getLogger(FirmaOTPService.class);
    ...
    public FirmaOTPService(TimestampService timestampService, PdfSignatureService pdfService,
                            MinioFirmaService minioService, DocumentoFirmadoRepository documentoRepository,
    ...
    @Transactional
    public Mono<FirmaResponse> firmar(FirmarOTPRequest request, String ip, String userAgent, 
                                       String geolocalizacion, String dispositivoId) {
        log.info("Iniciando firma OTP para: {}", request.nombreArchivo());
        ...
        docInicial.setNivelFirma(NivelFirma.OTP_DOBLE_CANAL);
        docInicial.setEstatus(EstatusFirma.EN_VALIDACION_OTP);
        ...
        boolean otpEmailValido = validarOtp(request.otpEmail());
        boolean otpSmsValido = validarOtp(request.otpSms());
        
        if (!otpEmailValido || !otpSmsValido) {
            docGuardado.setEstatus(EstatusFirma.RECHAZADA);
            docGuardado.setMotivoRechazo("OTP invalido o expirado");
            documentoRepository.save(docGuardado);
            return Mono.just(crearResponse(docGuardado, "OTP invalido"));
        }
        ...
        return timestampService.solicitarTimestamp(hashOriginal)
            .map(tsResp -> {
                ...
                guardarMetadata(docGuardado.getId(), request.otpEmail(), request.otpSms(), 
                               ip, userAgent, geolocalizacion, dispositivoId);
                ...
                return crearResponse(docGuardado, "Firma OTP completada");
            })
            .onErrorResume(e -> {
                log.error("Error en firma OTP: {}", e.getMessage());
                docGuardado.setEstatus(EstatusFirma.ERROR);
    ...
    private void guardarMetadata(UUID documentoId, String otpEmail, String otpSms,
                                  String ip, String ua, String geo, String dispositivo) {
        ...
        meta.setGeolocalizacion(geo);
        meta.setDispositivoId(dispositivo);
        meta.setOtpHash(calcularHash(otpEmail + ":" + otpSms));
